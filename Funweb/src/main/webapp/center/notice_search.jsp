...
//-----------------------------------------
//BoardDAO 객체의 selectBoardList() 메서드를 호출하여
//검색어에  해당하는게시물 목록 조회 작업 요청
...
// 				}
			...
			<div class="clear"></div>
			<div id="page_control">
				<!-- 페이징 처리 시 링크에 notice_search.jsp 로 이동(검색어도 전달) -->
				<!-- 
				...
				<a
					href="notice_search.jsp?page=<%=pageNum - 1%>&search=<%=search %>&searchType=<%=searchType%>">Prev</a>
				...
				<%} else { %>
				<a
					href="notice_search.jsp?page=<%=i%>&search=<%=search %>&searchType=<%=searchType%>"><%=i %></a>
				<%} %>
				<%} %>
				<!-- 
				...
				<a
					href="notice_search.jsp?page=<%=pageNum + 1%>&search=<%=search %>&searchType=<%=searchType%>">Next</a>
				<%} else { // 이전페이지가 존재하지 않을 경우 %>
				&nbsp;Next
				<%} %>
			</div>
